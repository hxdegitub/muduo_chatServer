...
// 从配置文件中加载配置项
auto ConnectionPool::loadConfigFile() -> bool {
  FILE *pf = fopen("/home/hx/chatServer/bin/mysql.ini", "r");
  if (pf == nullptr) {
...
ConnectionPool::ConnectionPool() {
  ...
  thread produce([this] { this->produceConnectionTask(); });
  produce.detach();

  // 启动一个新的定时线程，扫描超过maxIdleTime时间的空闲连接，进行对于的连接回收
  thread scanner([this] { this->scannerConnectionTask(); });
  scanner.detach();
}
...
auto ConnectionPool::getConnection() -> shared_ptr<Connection> {
  unique_lock<mutex> lock(_queueMutex);
  while (_connectionQue.empty()) {
    // sleep
